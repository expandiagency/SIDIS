...
var resizableSwiper = (breakpoint, swiperElementOrClass, swiperSettings, callback) => {
	const el = typeof swiperElementOrClass === "string" ? document.querySelector(swiperElementOrClass) : swiperElementOrClass;
	if (!el) return;
	const media = window.matchMedia(breakpoint);
	const enable = () => {
		if (el.swiper) return;
		const instance = new Swiper(el, swiperSettings);
		el.swiper = instance;
	...
	media.addEventListener("change", checker);
	checker();
};
function initSliders() {
	if (document.querySelector(".why__slider")) document.querySelectorAll(".why__slider").forEach((el) => {
	...
	if (document.querySelector(".automation__slider")) document.querySelectorAll(".automation__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		new Swiper(el, {
	...
	if (document.querySelector(".solutions__slider")) {
		const sharedNext = document.querySelector(".solutions .swiper-button-next");
		const sharedPrev = document.querySelector(".solutions .swiper-button-prev");
		const sharedPagination = document.querySelector(".solutions .swiper-pagination");
		const instances = [];
		document.querySelectorAll(".solutions__slider").forEach((el) => {
		...
		function getSwiperByTabItem(tabItem) {
			const el = tabItem.querySelector(".solutions__slider");
			if (!el) return null;
			return instances.find((swiper) => swiper.el === el) || null;
		}
		function updateActiveSwiper(activeSwiper) {
			...
			activeSwiper.params.navigation.prevEl = sharedPrev;
			activeSwiper.params.navigation.nextEl = sharedNext;
			activeSwiper.navigation.init();
			activeSwiper.navigation.update();
			activeSwiper.params.pagination.el = sharedPagination;
			activeSwiper.pagination.init();
			activeSwiper.pagination.render();
			activeSwiper.pagination.update();
			requestAnimationFrame(() => {
				activeSwiper.updateSize();
				activeSwiper.updateSlides();
				activeSwiper.updateProgress();
				activeSwiper.updateSlidesClasses();
				activeSwiper.update();
				activeSwiper.slideTo(0, 0, false);
			});
		}
		document.querySelectorAll("[data-fls-tabs]").forEach((tabsBlock) => {
			const activeItem = tabsBlock.querySelector("[data-fls-tabs-item]:not([hidden])");
			if (!activeItem) return;
			updateActiveSwiper(getSwiperByTabItem(activeItem));
		});
		document.querySelectorAll("[data-fls-tabs-title]").forEach((title) => {
			title.addEventListener("click", () => {
				const tabsBlock = title.closest("[data-fls-tabs]");
				if (!tabsBlock) return;
				requestAnimationFrame(() => {
					requestAnimationFrame(() => {
						const activeItem = tabsBlock.querySelector("[data-fls-tabs-item]:not([hidden])");
	...
	if (document.querySelector(".reviews__slider")) document.querySelectorAll(".reviews__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		const pagination = parent.querySelector(".swiper-pagination");
		const isInInner = el.closest(".inner");
		new Swiper(el, {
			modules: [Navigation, Pagination],
			observer: true,
			observeParents: true,
	...
	if (document.querySelector(".articles__slider")) document.querySelectorAll(".articles__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		const pagination = parent.querySelector(".swiper-pagination");
	...
	if (document.querySelector(".features__slider")) document.querySelectorAll(".features__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		const pagination = parent.querySelector(".swiper-pagination");
		resizableSwiper("(width < 48em)", el, {
			modules: [Navigation, Pagination],
			observer: true,
			observeParents: true,
			slidesPerView: 3,
	...
	if (document.querySelector(".article-block__slider")) document.querySelectorAll(".article-block__slider").forEach((el) => {
		new Swiper(el, {
			modules: [Navigation],
			observer: true,
			observeParents: true,
	...
	if (document.querySelector(".tech__slider")) document.querySelectorAll(".tech__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		const pagination = parent.querySelector(".swiper-pagination");
	...
	if (document.querySelector(".cases__slider")) document.querySelectorAll(".cases__slider").forEach((el) => {
		const parent = el.parentElement;
		const next = parent.querySelector(".swiper-button-next");
		const prev = parent.querySelector(".swiper-button-prev");
		const pagination = parent.querySelector(".swiper-pagination");
...
function menuBurgerInit() {
	...
	document.addEventListener("click", function(e) {
		const burgerBtn = e.target.closest("[data-menu-burger]");
		const overlayClick = e.target.closest(".menu-overlay");
		if (burgerBtn) if (html.hasAttribute("data-menu-open")) {
			html.removeAttribute("data-menu-open");
			lockManager.unlock("burger");
		} else {
			html.setAttribute("data-menu-open", "");
			lockManager.lock("burger");
		}
		if (overlayClick) {
			if (html.hasAttribute("data-menu-open")) {
				html.removeAttribute("data-menu-open");
				lockManager.unlock("burger");
			}
		}
...
document.querySelector("[data-menu-burger]") && window.addEventListener("load", menuBurgerInit);
...
window.initMenu = function({ bodyLock: enableBodyLock = false } = {}) {
	...
	let useBodyLock = enableBodyLock;
	...
	createOverlay();
	function closeAllMenus({ preserveLock = false } = {}) {
		...
		if (!preserveLock && useBodyLock) lockManager.unlock("menu");
	...
	function openMenu(menuName, { withFocusReturn = true } = {}) {
		clearTimeout(closeTimeout);
		closeAllMenus({ preserveLock: true });
		...
		if (menu && buttons.length) {
			...
			if (useBodyLock) lockManager.lock("menu");
		}
	}
	...
	if (!isTouch && !isMobile.any()) {
		...
		document.querySelectorAll("[data-menu]").forEach((button) => {
			...
			const possiblyActivate = () => {
				clearTimeout(closeTimeout);
				if (isMovingToMenu(menu)) closeTimeout = setTimeout(possiblyActivate, DELAY);
			...
			button.addEventListener("mouseleave", () => {
				closeTimeout = setTimeout(closeAllMenus, DELAY);
			});
...
window.initSubmenu = function() {
	...
	const html = document.documentElement;
	const isTouch = "ontouchstart" in window || navigator.maxTouchPoints > 0;
	const breakpoint = window.matchMedia("(max-width: 62em)");
	function getRootAttr(el) {
		return [...el.attributes].map((a) => a.name).find((name) => /^data-submenu(-\d+)?$/.test(name));
	}
	function makeCfg(rootAttr) {
		return {
			root: rootAttr,
			link: `${rootAttr}-link`,
			item: `${rootAttr}-item`,
			close: `${rootAttr}-close`,
			links: `${rootAttr}-links`,
			items: `${rootAttr}-items`
		};
	}
	function makeManager() {
		return {
			active: null,
			setActive(inst) {
				if (this.active && this.active !== inst) this.active.close();
				this.active = inst;
			},
			clear(inst) {
	...
	class Submenu {
		constructor(root, cfg, manager) {
			this.root = root;
			this.cfg = cfg;
			this.manager = manager;
			this.openedByClick = false;
			this.linksWrap = root.querySelector(`[${cfg.links}]`);
			this.itemsWrap = root.querySelector(`[${cfg.items}]`);
			this.state = { active: null };
			this.hoverTimer = null;
			this.mouseHistory = [];
			this.bindEvents();
			if (!isTouch && !isMobile.any() && breakpoint) this.activateFirst();
		}
		bindEvents() {
			const { cfg } = this;
			this.root.addEventListener("click", (e) => {
				const btn = e.target.closest(`[${cfg.link}]`);
				const closeBtn = e.target.closest(`[${cfg.close}]`);
				if (btn && this.root.contains(btn)) {
					const name = btn.getAttribute(cfg.link);
					if (btn.hasAttribute(`${cfg.root}-click`) || isTouch && isMobile.any()) e.preventDefault();
					const byClick = btn.hasAttribute(`${cfg.root}-click`);
					this.state.active === name ? this.close() : this.open(name, byClick);
					return;
				}
		...
		initHover() {
			...
			let overMenu = false;
			this.root.querySelectorAll(`[${cfg.link}]`).forEach((btn) => {
				if (btn.hasAttribute(`${cfg.root}-click`)) return;
				const name = btn.getAttribute(cfg.link);
				btn.addEventListener("mouseenter", () => {
					overButton = true;
					clearTimeout(this.hoverTimer);
					this.hoverTimer = setTimeout(() => this.open(name), 120);
				});
			...
			bindArea(this.itemsWrap);
			this.root.querySelectorAll(`[${cfg.item}]`).forEach(bindArea);
			this.startCloseTimer = () => {
				clearTimeout(this.hoverTimer);
				this.hoverTimer = setTimeout(() => {
					if (!overButton && !overMenu && !this.openedByClick) this.close();
				}, 200);
			};
		}
		getBtnPosition(btn) {
		...
		positionItem(item, btn) {
			const p = this.getBtnPosition(btn);
			...
			if (left < 8) left = 8;
			item.style.setProperty("--submenu-left", `${left}px`);
			item.style.setProperty("--submenu-btn-left", `${p.left}px`);
			item.style.setProperty("--submenu-btn-right", `${p.right}px`);
			item.style.setProperty("--submenu-btn-bottom", `${p.bottom}px`);
		}
		open(name, byClick = false) {
			this.openedByClick = byClick;
			this.manager.setActive(this);
			if (this.state.active && this.state.active !== name) this.deactivate(this.state.active);
			const { cfg } = this;
			const btn = this.root.querySelector(`[${cfg.link}="${name}"]`);
			const item = this.root.querySelector(`[${cfg.item}="${name}"]`);
			if (!btn || !item) return;
			this.positionItem(item, btn);
			btn.classList.add(BTN_ACTIVE);
			item.classList.add(ACTIVE);
			this.linksWrap?.classList.add(BTN_ACTIVE);
			this.itemsWrap?.classList.add(ACTIVE);
			if (!this.cfg.root.match(/-\d+$/)) html.classList.add("submenu-open");
			...
			if (window.enableFocusTrap) this.removeTrap = enableFocusTrap(item, { openClass: ACTIVE });
		}
		deactivate(name) {
			const { cfg } = this;
			this.root.querySelector(`[${cfg.link}="${name}"]`)?.classList.remove(BTN_ACTIVE);
			this.root.querySelector(`[${cfg.item}="${name}"]`)?.classList.remove(ACTIVE);
			html.classList.remove(`submenu-open--${name}`);
		}
		activateFirst() {
			const { cfg } = this;
			const firstBtn = this.root.querySelector(`[${cfg.link}]`);
			if (!firstBtn) return;
			const name = firstBtn.getAttribute(cfg.link);
			if (!name) return;
			const item = this.root.querySelector(`[${cfg.item}="${name}"]`);
			if (!item) return;
			this.positionItem(item, firstBtn);
			firstBtn.classList.add(BTN_ACTIVE);
			item.classList.add(ACTIVE);
			this.linksWrap?.classList.add(BTN_ACTIVE);
			this.itemsWrap?.classList.add(ACTIVE);
			this.state.active = name;
		...
		close() {
			...
			if (!this.cfg.root.match(/-\d+$/)) html.classList.remove("submenu-open");
			if (this.removeTrap) {
				this.removeTrap();
				this.removeTrap = null;
			}
	...
	document.querySelectorAll(selector).forEach((el) => {
		...
		groups.get(attr).els.push(el);
	});
	groups.forEach(({ cfg, manager, els }) => {
		els.forEach((el) => new Submenu(el, cfg, manager));
	});
};
...
function headerScroll() {
	const headerScrollBlocks = document.querySelectorAll("[data-fls-header-scroll]");
	if (headerScrollBlocks.length) headerScrollBlocks.forEach((header) => {
		...
		document.addEventListener("scroll", function(e) {
			...
			if (scrollTop >= startPoint) {
				...
				setTimeout(() => {
					!header.classList.contains("--header-fixed") && header.classList.add("--header-fixed");
				}, headerShowTimer);
				if (headerShow) if (scrollTop > scrollDirection) header.classList.contains("--header-show") && header.classList.remove("--header-show");
				else {
				...
				setTimeout(() => {
					header.classList.contains("--header-fixed") && header.classList.remove("--header-fixed");
				}, headerShowTimer);
...
window.customMiniSelect = function() {
	...
	selParents.forEach((selBlock) => {
		...
		const selAllBtn = selBlock.querySelector("[data-sel-block-all]");
		let isOpen = false;
		...
		function checkAndSetPlaceholder() {
			...
			const activeOption = Array.from(selOptions).find((o) => o.classList.contains("is-active"));
			if (!activeOption) {
				setPlaceholder();
				return;
			}
			if (selAllBtn && activeOption === selAllBtn) {
				setPlaceholder();
				return;
			}
...
customMiniSelect();
if (document.querySelector("[data-switch-block]")) document.querySelectorAll("[data-switch-block]").forEach((block) => {
	const buttons = block.querySelectorAll("button");
	if (!buttons.length) return;
	const bg = document.createElement("span");
	if (block.getAttribute("data-switch-block") === "border") bg.className = "switch-bg switch-bg-border";
	else bg.className = "switch-bg";
	...
	const activeBtn = block.querySelector(".is-active") || buttons[0];
	...
	window.addEventListener("resize", () => {
		const current = block.querySelector(".is-active");
...
if (document.querySelectorAll("[data-video-controlls]").length) document.querySelectorAll("[data-video-controlls]").forEach((container) => {
	const video = container.querySelector("video");
	const muteBtn = container.querySelector("[data-video-controlls-mute]");
	const progress = container.querySelector("[data-video-controlls-progress]");
	const fullscreenBtn = container.querySelector("[data-video-controlls-full]");
	const parent = video.closest("[data-video-controlls-parent]");
	const playBtn = parent.querySelector("[data-video-controlls-play]");
	let rafId = null;
	parent?.classList.add("video-paused");
	...
	fullscreenBtn.addEventListener("click", () => {
		if (video.webkitEnterFullscreen) video.webkitEnterFullscreen();
			...
			parent.classList.remove("fullscreen-video");
		} else if (container.requestFullscreen) {
			container.requestFullscreen();
			container.classList.add("fullscreen-video");
			parent.classList.add("fullscreen-video");
		}
	});
...
var scrollToTop = (behavior = "smooth") => {
	window.scrollTo({
		top: 0,
		behavior
	});
};
var dataGoTop = document.querySelector("[data-go-top]");
if (dataGoTop) dataGoTop.addEventListener("click", () => {
	scrollToTop();
});
window.hoverFooterLinks = function() {
	const footerMenuLinks = document.querySelectorAll(".footer__menu-link");
	if (footerMenuLinks.length) footerMenuLinks.forEach((link) => {
		link.addEventListener("mouseenter", () => {
			footerMenuLinks.forEach((otherLink) => {
				if (otherLink !== link) otherLink.classList.add("is-faded");
			});
		});
		link.addEventListener("mouseleave", () => {
			footerMenuLinks.forEach((otherLink) => {
				otherLink.classList.remove("is-faded");
			});
		});
	});
};
window.hoverFooterLinks();
function buildLocalNavigation() {
	...
	sections.forEach((section) => {
		...
		const links = document.querySelectorAll(`[data-fls-scrollto="#${id}"]`);
		if (links.length) linkMap.set(section, links);
	});
	let ticking = false;
	let lastActiveSection = null;
	const update = () => {
		const offset = header ? header.offsetHeight : 140;
		const scroll_y = window.pageYOffset + offset;
		let currentActiveSection = null;
		sections.forEach((current) => {
			const section_height = current.offsetHeight;
			const section_top = current.getBoundingClientRect().top + window.pageYOffset - 70;
			const links = linkMap.get(current);
			if (!links) return;
			if (scroll_y > section_top && scroll_y <= section_top + section_height) currentActiveSection = current;
			links.forEach((link) => {
		...
		let sectionToActivate = currentActiveSection || lastActiveSection;
		if (!sectionToActivate && sections.length) {
			sectionToActivate = sections[0];
			lastActiveSection = sectionToActivate;
		}
		if (sectionToActivate) {
			const links = linkMap.get(sectionToActivate);
...
if (document.querySelectorAll("[data-scroll-section]").length && document.querySelectorAll("[data-fls-scrollto]").length) buildLocalNavigation();
function articleMenuInit() {
	document.addEventListener("click", function(e) {
		if (e.target.closest("[data-article-menu-btn]")) document.documentElement.toggleAttribute("data-article-menu-open");
		else if (!e.target.closest("[data-article-menu-btn]") && !e.target.closest("[data-article-menu]")) document.documentElement.removeAttribute("data-article-menu-open");
	});
}
document.querySelector("[data-article-menu-btn]") && window.addEventListener("load", articleMenuInit);
function textOpacityScroll() {
	const items = document.querySelectorAll("[data-scroll-text]");
	...
	items.forEach((item) => {
		const itemValue = item.querySelector("[data-scroll-text-value]");
		const itemMask = item.querySelector("[data-scroll-text-mask]");
		if (!itemValue || !itemMask) {
		...
		const originalHTML = itemValue.innerHTML;
		function wrapWordsPreservingBr(html) {
			const wrapper = document.createElement("div");
			wrapper.innerHTML = html;
			const fragment = document.createDocumentFragment();
			function processNode(node) {
				if (node.nodeType === Node.TEXT_NODE) node.textContent.split(/(\s+)/).forEach((part) => {
					if (part.trim()) {
						const span = document.createElement("span");
						span.style.transition = `opacity ${itemSpeed}ms`;
						span.style.opacity = itemOpacity;
						span.textContent = part;
						fragment.appendChild(span);
					} else fragment.appendChild(document.createTextNode(part));
				});
				else if (node.nodeName === "BR") fragment.appendChild(document.createElement("br"));
					...
					Array.from(node.childNodes).forEach((child) => {
						...
						const processed = wrapWordsPreservingBr(childWrapper.innerHTML);
						inner.appendChild(processed);
					});
					cloned.appendChild(inner);
		...
		itemValue.innerHTML = "";
		itemValue.appendChild(wrapWordsPreservingBr(originalHTML));
		const itemWords = itemValue.querySelectorAll("span");
		const totalWords = itemWords.length;
		const handleScroll = throttle(() => {
			const maskRect = itemMask.getBoundingClientRect();
			const windowHeight = window.innerHeight;
			let progress = 0;
			if (maskRect.top < windowHeight) {
				const elementHeight = maskRect.height;
				const startPoint = windowHeight - windowHeight / 4;
				const endPoint = -elementHeight + windowHeight / 2;
				progress = (startPoint - maskRect.top) / (startPoint - endPoint);
				progress = Math.max(0, Math.min(1, progress));
			} else if (maskRect.top >= windowHeight) progress = 0;
			else progress = 1;
			updateWordsOpacity(itemWords, Math.floor(progress * totalWords), itemOpacity);
		}, 16);
		function updateWordsOpacity(words, visibleCount, baseOpacity) {
			words.forEach((word, index) => {
				if (index < visibleCount) word.style.opacity = "1";
				else word.style.opacity = baseOpacity;
			});
		}
...
document.addEventListener("DOMContentLoaded", textOpacityScroll);
//#endregion
...
function tabs() {
			...
			tabsContent.forEach((tabsContentItem, index) => {
				if (tabsTitles[index].classList.contains("--tab-active")) {
					if (tabsBlockAnimate) {
						...
						setTimeout(() => updateSwiper(tabsContentItem), tabsBlockAnimate);
					} else {
						tabsContentItem.hidden = false;
						updateSwiper(tabsContentItem);
					}
	...
	function updateSwiper(container) {
		container.querySelectorAll(".swiper").forEach((swiperEl) => {
			...
			swiper.update();
			swiper.slideToLoop ? swiper.slideToLoop(0, 0, false) : swiper.slideTo(0, 0, false);
		});
	}
}
