<?php
/**
 * Seed all case studies with full content from case.html structure.
 * Visit: https://sidis.expandi.agency/cases_patch.php
...
// Cases to patch
$case_slugs = ['apex-agency-invoice','nda-data-insights','cloud-origin-extraction','innovate-healthcare'];

// Also try all active cases from DB
...
foreach ($all_cases as $c) {
    ...
    if (empty($ct['extras'])) {
        ...
        echo "Extras (solution, results, testimonial) saved.\n";
    } else {
    ...
    $ch_count = row('SELECT COUNT(*) as c FROM case_challenges WHERE case_id=?', [$case_id])['c'];
    if ((int)$ch_count === 0) {
    ...
    if ((int)$tech_count === 0) {
        ...
        echo "Tech items already exist.\n";
    }

    // Seed services if none
    $svc_count = row('SELECT COUNT(*) as c FROM case_services WHERE case_id=? AND lang_id=?', [$case_id,$lang_id])['c'];
    if ((int)$svc_count === 0) {
